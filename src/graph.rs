...
pub fn create_hourly_graph(
    ...
    // Animation tick: advance lerp and breathing; redraw each frame
    {
        ...
        area.add_tick_callback(move |area, _clock| {
            ...
            // Throttle breathing animation to ~30FPS to save CPU when idle
            // If animating data transition, run full speed
        ...
        area.set_draw_func(move |_widget, ctx, w, h| {
            ...
            let draw_static = |c: &cairo::Context| {
                ...
                if count == 0 {
                 ...
                 // Temps Calcs for Y-axis
                let mut temp_min = data_for_draw.iter().map(|h| h.temp).fold(f64::INFINITY, f64::min);
                let mut temp_max = data_for_draw.iter().map(|h| h.temp).fold(f64::NEG_INFINITY, f64::max);
                if !temp_min.is_finite() || !temp_max.is_finite() { temp_min = 0.0; temp_max = 1.0; }
                temp_min -= 2.0;
                temp_max += 2.0;
                ...
                let temp_to_y = |t: f64| -> f64 { temp_top + (temp_max - t) / temp_range * (temp_bot - temp_top) };

                // X positions
                let dx = if count == 1 { 0.0 } else { plot_width / (count as f64 - 1.0) };
                let mut xs = Vec::with_capacity(count);
                for i in 0..count {
                    let x = if count == 1 { left_margin + plot_width / 2.0 } else { left_margin + dx * i as f64 };
                    xs.push(x);
                }

                // Day/Night Shading
                let mut shade_dx = dx;
                if count == 1 { shade_dx = plot_width; }
                for i in 0..count {
                ...
                for t in ticks {
                    ...
                    c.move_to(left_margin, y_tick);
                    c.line_to(width - right_margin, y_tick);
                }
                let _ = c.stroke();
                ...
                c.set_source_rgba(tick_color.0, tick_color.1, tick_color.2, tick_color.3);
                c.set_line_width(1.0);
                c.select_font_face("Sans", cairo::FontSlant::Normal, cairo::FontWeight::Normal);
                c.set_font_size(10.0);
                for t in ticks {
                    let y_tick = temp_to_y(t);
                    c.move_to(left_margin - 6.0, y_tick);
                    c.line_to(left_margin - 2.0, y_tick);
                    let _ = c.stroke();
    
                    let text = format!("{:.0}°", t);
                    if let Ok(ext) = c.text_extents(&text) {
                        c.set_source_rgb(tick_label_color.0, tick_label_color.1, tick_label_color.2);
                        c.move_to(left_margin - 8.0 - ext.width(), y_tick + ext.height() / 2.0);
                        let _ = c.show_text(&text);
                    }
                    c.set_source_rgba(tick_color.0, tick_color.1, tick_color.2, tick_color.3);
                }
            ...
            if count > 0 {
                 // Recalculate geometry (needed for dynamic parts too)
                let mut temp_min = data_for_draw.iter().map(|h| h.temp).fold(f64::INFINITY, f64::min);
                let mut temp_max = data_for_draw.iter().map(|h| h.temp).fold(f64::NEG_INFINITY, f64::max);
                if !temp_min.is_finite() || !temp_max.is_finite() { temp_min = 0.0; temp_max = 1.0; }
                temp_min -= 2.0;
                ...
                for h in data_for_draw.iter().take(count) {
                    y_temps_target.push(temp_to_y(h.temp));
                ...
                if let Some(x) = now_x {
                    ...
                    ctx.arc(x, temp_top + (temp_bot - temp_top) * 0.5, pulse, 0.0, std::f64::consts::PI * 2.0);
                    let _ = ctx.fill();
                }
            }
        });
